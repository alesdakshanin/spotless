...
**Reason**: Replaced by the apply bar summary. With flat cards showing all decisions inline, a separate counter row is redundant.
**Migration**: The apply bar displays "Replacing N, removing N" which covers the essential counts.

### Requirement: No-match track removal
**Reason**: Replaced by the unified remove action row. Every card (with or without candidates) now has a "Remove from {source}" row.
**Migration**: The remove row on cards without candidates serves the same purpose.

### Requirement: Checkbox color differentiation
**Reason**: Checkboxes are eliminated. Selection is indicated by row highlighting (green for replace, red for remove).
**Migration**: Row highlight colors serve the same differentiation purpose.

### Requirement: Row expansion for candidate selection
**Reason**: Accordion expansion is eliminated. All candidates are visible inline in the card body.
**Migration**: Candidates are always-visible clickable rows within each card.

### Requirement: Filter bar
**Reason**: With flat cards showing all options inline, filtering is unnecessary. All decisions are visible at a glance.
**Migration**: Users visually scan the card list — cards with no green-highlighted row need attention.

### Requirement: Select all control
**Reason**: Bulk actions are removed. Auto-proposal handles the high-confidence case. Remaining tracks benefit from individual review.
**Migration**: Auto-proposal pre-selects high-confidence matches. Users review remaining cards individually.
